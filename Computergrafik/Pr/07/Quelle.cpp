...
void drawCone(float xcoord, float ycoord, float zcoord, float color[3]){
	...
	if (bezierPoints.back() != NULL){
		vert * v = bezierPoints.back();
		float winkel = acos( (xcoord*v->x+ycoord*v->y+zcoord* ) );
...
void loadObjectFile(){
	readFile("C:/obj/raumschiff4.obj");
...
int main(int argc, char** argv)
{
	static bool read = 0;
	if (!read){
		readFileBezier("C:/obj/beziercurve.obj");
		read = 1;
	}
