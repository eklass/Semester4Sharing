...
#define OBJPATH1 "/Users/erichklassen/Documents/Studium/Uni/Bielefeld/Semester4/Computergrafik/Pr/08..eig.07/obj-Files/RoboMitTexasHutRichtig.obj"
#define OBJPATH2 "/Users/erichklassen/Documents/Studium/Uni/Bielefeld/Semester4/Computergrafik/Pr/08..eig.07/obj-Files/SpaceBalls.obj"
#define SPEED 0.5f
...
vector<HE_edge*> edgeList2;
vector<HE_vert*> vertexList2;
vector<HE_face*> faceList2;



static int howManyNormals = 0;
...
void fillLists(string input){
	...
	//    else if (regex_match(input, face)){
		...
		for (unsigned int i = 1; i < splitted.size(); i++){
			...
			vertList_HEAD[stoi(splitted[i]) - 1]->norm = normList[stoi(splitted2[2]) - 1];
...
void buildHalfEdge2(){
	...
	for (FACE_LIST* F : faceList_HEAD){
		...
		if (Vertices.find(tuple<float, float, float>(vertList_HEAD[F->vertices.back()]->x, vertList_HEAD[F->vertices.back()]->y, vertList_HEAD[F->vertices.back()]->z)) == Vertices.end()){ // if vertex doesnt exist yet:
			...
			newVert->norm = vertList_HEAD[F->vertices.back()]->norm;
			vertexList.push_back(newVert);
...
void buildHalfEdge2SecondObj(){
	map< pair<int, int>, HE_edge* > Edges;
	map< tuple<float, float, float>, HE_vert*> Vertices;
	// faceList hingegen ist eine globale (vorerst leere) Liste die Faces h獲t
	for (FACE_LIST* F : faceList_HEAD){
		HE_face* localFace = new HE_face;
		faceList2.push_back(localFace);
		//Half-Edges erstellen:   (n,n+1) = new edge, (n,m) = new edge, (m,n) = new edge; n>m;
		for (unsigned int u = 0; u < F->vertices.size() - 1; u++){
			Edges[pair<int, int>(F->vertices[u], F->vertices[u + 1])] = new HE_edge;
			HE_edge * localEdge = Edges[pair<int, int>(F->vertices[u], F->vertices[u + 1])];
			localEdge->face = localFace;
			edgeList2.push_back(localEdge);
...
void drawMagic(){
	...
	for (HE_face *f : faceList){
		...
		HE_edge * edd = f->edge;
		do{
			glNormal3f(edd->vert->norm->x, edd->vert->norm->y, edd->vert->norm->z);
...
void toggleLight(){
	...
	GLfloat light_pos[] = { 2.0, 2.0, 0.0, 0.0 };
	GLfloat light_pos2[] = { -2.0, 2.0, 0.0, 0.0 };
...
int main(int argc, char** argv)
{
	...
	if (modus == BEZIERMODEOFF){
		...
		if (loadHES){
			...
			static bool read = 0;
			if (!read){
				//vector<HE_edge*> edgeList, vector<HE_vert*> vertexList, vector<HE_face*> faceList, vector<HE_norm*> normList
				...
				readFile(OBJPATH2);
				buildHalfEdge2SecondObj();
				//printStatistics();

				cout << "How Many Normals: " << howManyNormals << endl;
				read = 1;
			}
			loadHES = false;
		}
