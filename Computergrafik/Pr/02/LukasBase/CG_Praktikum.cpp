...
//    glRotatef(-rot_sphere1, 0, 1, 0); //
    ...
    glColor3f(0, 1, 1);
    glPushMatrix();
    glRotatef(rot_sphere1, 0, 1, 0); // 45° Neigung
    glTranslatef(2.0f,0,0); // Verschiebung
    glRotatef(-rot_sphere1, 0, 0, 1); // 45° Neigung
    ...
    rot_sphere1+=1;
    //std::cout<<(int)rot_sphere1<<std::endl;

    zFaktor+=0.1;
    while(zFaktor>=3.75) {
        zFaktor-=3.75;
    }
    while(rot_sphere1>=360.0) {
        rot_sphere1-=360.0;
    }
